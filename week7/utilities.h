...
{
    float highest = dataArray[0].bloodIron;
    for(int c = 0; c < numReadings; c++) {
        if(highest<dataArray[c].bloodIron){
            highest = dataArray[c].bloodIron;
        } 
   }
...
float find_lowest(reading* dataArray, int numReadings)
{
    ...
    for(int c = 0; c < numReadings; c++) {
        if(lowest>dataArray[c].bloodIron){
            lowest = dataArray[c].bloodIron;
        } 
   }
...
{
    for (int c = 0; c<numReadings; c++){
        if (strstr(dataArray[c].date, month)){
            printf("%s - Blood iron: %.1f\n", dataArray[c].date, dataArray->bloodIron);
        }
    }
}

float calcSD(reading* dataArray, float mean, float sum, int numReadings)
{
    float SD;
    for (int i = 0; i < numReadings; i++) {
        SD += pow(dataArray[i].bloodIron - mean, 2);
    }
    return SD;
}